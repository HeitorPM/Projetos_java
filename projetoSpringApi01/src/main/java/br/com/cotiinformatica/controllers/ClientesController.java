...
public class ClientesController {
	...
	public ResponseEntity<ClienteResponse> put(@RequestBody ClientePutRequest request) {
		...
		try {
			...
			// verificar se o cliente existe na base de dados pelo id, obs: findById vai
			// retornar um optional
			Optional<Cliente> item = clienteRepository.findById(request.getIdCliente());

			// verificar se o cliente nao existe na base
			if (item.isEmpty()) {
			...
			// bverificar se o email informado na edicao pertence a outro cliente na base
			...
			if (consultaPorEmail != null && consultaPorEmail.getIdCliente() != cliente.getIdCliente()) {
				...
				response.setMensagem("O email informado já esta cadastrado para outro cliente.");

				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
			}

			// verificar se o email informado na edicao pertence a outro cliente na base
			...
			if (consultaPorCpf != null && consultaPorCpf.getIdCliente() != cliente.getIdCliente()) {
				response.setStatus(HttpStatus.BAD_REQUEST.value());
				response.setMensagem("O CPF informado já esta cadastrado para outro cliente.");

				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
			}

			// modificando os dados do clinete
			cliente.setNome(request.getNome());
			cliente.setEmail(request.getEmail());
			cliente.setCpf(request.getCpf());
			cliente.setTelefone(request.getTelefone());
			...
			/*
			 * atualizando o cliente na base de dados, usa o save tanto pra cadastrar,
			 * quanto pra atualizar, o que faz a diferença é se o objeto tem ID ou n, se n
			 * tiver cadastra, se tiver atualiza
			 * 
			 */
			clienteRepository.save(cliente);
	...
	// método para gerar um serviço de deleção de clientes
	// esse value = ENDPOINT + "/{id}" seria equivalente a HTTP DELeTE
	// http://localhost:8083/ -->api/clientes/1
	// manda o id na propria url
	...
	// método para gerar um serviço de seleção de clientes
	...
	public ResponseEntity<List<Cliente>> getAll() {
		...
		try {
			...
			// (List<Cliente>) serve como cast, pq o findALl() retorna um Iterable
	...
	// método para gerar um serviço de seleção de clientes baseado no ID
	...
	public ResponseEntity<Cliente> getById(@PathVariable("id") Integer id) {
		...
		try {
			...
			// buscar cliente no banco de dados atraves do id
			Optional<Cliente> item = clienteRepository.findById(id);
