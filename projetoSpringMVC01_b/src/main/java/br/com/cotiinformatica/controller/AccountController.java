...
public class AccountController {
	...
	public ModelAndView autenticarUsuario(AccountLoginDTO dto, HttpServletRequest request) {
		...
		try {
			...
			if (usuario != null) {
				...
				// modelAndView.addObject("mensagem_sucesso", "Autenticação realizada com
				// sucesso");
	...
	public ModelAndView recuperarSenha(AccountPasswordRecoverDTO dto) {
		...
		try {
			...
			//gerar um nova senha para o usuario
			usuario.setSenha(String.valueOf(new Random().nextInt(999999999)));
			
			//enviando um email par ao usuario com a nova senha
			EmailService.enviarMensagem(usuario.getEmail(), "Nova senha gerada com sucesso", 
					 "Olá, " + usuario.getNome() + 
	...
	public ModelAndView logout(HttpServletRequest request) {
		...
		// apagar os dados gravados em sessao
